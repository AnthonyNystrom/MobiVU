...
int sws_scale(SwsContext *c, uint8_t* src[], int srcStride[], int srcSliceY,
    ...
    if (c->sliceDir == 1) {
        ...
        int srcStride2[4];
        int dstStride2[4];
        srcStride2[0] = srcStride[0];
        srcStride2[1] = srcStride[1];
        srcStride2[2] = srcStride[2];
        dstStride2[0] = dstStride[0];
        dstStride2[1] = dstStride[1];
        dstStride2[2] = dstStride[2];
#endif
        ...
        int srcStride2[4];
        int dstStride2[4];
        dst2[0] = dst[0] + (c->dstH-1)*dstStride[0];
        dst2[1] = dst[1] + ((c->dstH>>c->chrDstVSubSample)-1)*dstStride[1];
        dst2[2] = dst[2] + ((c->dstH>>c->chrDstVSubSample)-1)*dstStride[2];
        srcStride2[0] = -srcStride[0];
        srcStride2[1] = -srcStride[1];
        srcStride2[2] = -srcStride[2];
        dstStride2[0] = -dstStride[0];
