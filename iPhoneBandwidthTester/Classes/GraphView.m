...
CGColorRef graphXColor()
{
	...
	if(c == NULL)
	{
		c = CreateDeviceRGBColor(0.0, 1.0, 0.0, 0.75);
	}
	return c;
}
...
CGColorRef graphYColor()
{
	...
	if(c == NULL)
	{
		c = CreateDeviceRGBColor(1.0, 0.0, 0.0, 0.4);
	}
	return c;
}
...
{
//	for(CGFloat y = -248.5; y <= -8.5; y += 60.0)
	for(CGFloat y = -368.5; y <= 120; y += 60.0)
	{
...
-(id)init
{
	...
	if(self != nil)
	{
		...
		layer.bounds = CGRectMake(0.0, -244, 32.0, 248);
...
-(void)drawLayer:(CALayer*)l inContext:(CGContextRef)context
{
	...
	{
		lines[i*2].x = i;
		lines[i*2].y = -xhistory[i] * IMUL - OFFSET;
		lines[i*2+1].x = i + 1;
		lines[i*2+1].y = -xhistory[i+1] * IMUL - OFFSET;
	}
	...
	for(i = 0; i < 32; ++i)
	{
		lines[i*2].y = -yhistory[i] * IMUL - OFFSET;
		lines[i*2+1].y = -yhistory[i+1] * IMUL - OFFSET;
	}
	...
	for(i = 0; i < 32; ++i)
	{
		lines[i*2].y = -zhistory[i] * IMUL - OFFSET;
		lines[i*2+1].y = -zhistory[i+1] * IMUL - OFFSET;
	}
...
-(void)drawRect:(CGRect)rect
{
	...
	[@"(ms)" drawInRect:CGRectMake(2.0, -248.0, 24.0, 16.0) withFont:systemFont lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentRight];
	[@"300" drawInRect:CGRectMake(2.0, -198.0, 24.0, 16.0) withFont:systemFont lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentRight];
	[@"200" drawInRect:CGRectMake(2.0, -138.0, 24.0, 16.0) withFont:systemFont lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentRight];
	[@"100" drawInRect:CGRectMake(2.0,  -78.0, 24.0, 16.0) withFont:systemFont lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentRight];
	[@"  0" drawInRect:CGRectMake(2.0,  -18.0, 24.0, 16.0) withFont:systemFont lineBreakMode:UILineBreakModeWordWrap alignment:UITextAlignmentRight];
}
...
//	text = [[GraphTextView alloc] initWithFrame:CGRectMake(0.0, 0.0, 32.0, 112.0)];
	text = [[GraphTextView alloc] initWithFrame:CGRectMake(0.0, 0.0, 32.0, 254.0)];
...
#define kSegmentInitialPosition CGPointMake(14.0, 130.0);
