...
CAudioStreamEngine* CAudioStreamEngine::NewL(CDemoVideoCallAppUi* aAppUi)
    {
...
CAudioStreamEngine* CAudioStreamEngine::NewLC(CDemoVideoCallAppUi* aAppUi)
    {
...
// onstructor
// ----------------------------------------------------------------------------
CAudioStreamEngine::CAudioStreamEngine(CDemoVideoCallAppUi* aAppUi)
...
void CAudioStreamEngine::OpenForPlay()
    ...
    iAudioBuffer.Zero();
    _bCanPlay=EFalse;
    //ShowMessage(_L("CAudioStreamEngine::OpenForPlay()"), ETrue);
    // if either stream is active, return
    if (iInputStatus!=ENotReady || iOutputStatus!=ENotReady) 
        {
        ShowMessage(_L("CAudioStreamEngine::OpenForPlay() Stream in use, cannot play audio."), ETrue);
        return;
        }
    ...
    /*if(!iBufferOK)
        {
        ShowMessage(_L("CAudioStreamEngine::Play() Nothing to play - record or load \na file first."), ETrue);
        return;
        }*/
...
void CAudioStreamEngine::Record()
    ...
    if (iInputStatus!=ENotReady || iOutputStatus!=ENotReady) 
    {
        ShowMessage(_L("CAudioStreamEngine::Record() Stream in use, \ncannot record audio."), ETrue);
        return;
    }
...
void CAudioStreamEngine::Stop()
    ...
    if (iInputStatus!=ENotReady) 
        {
        iInputStream->Stop();
        ShowMessage(_L("CAudioStreamEngine::Stop() Recording stopped!"), ETrue);
        iBufferOK = ETrue;
    ...
    if (iOutputStatus!=ENotReady) 
        ...
        ShowMessage(_L("CAudioStreamEngine::Stop() Playback stopped!"), ETrue);
        }
    }
...
void CAudioStreamEngine::SetEncodingL(TBool aAmr)
    ...
    if (iUseAMR != aAmr)
        ...
        if (iUseAMR)
            ...
            if (err != KErrNone)
                {
                ShowMessage(_L("CAudioStreamEngine::SetEncodingL AMR-NB not supported,\nusing PCM."), ETrue);    
                iCurrentEncoding = iDefaultEncoding;
                iUseAMR = EFalse;
                ...
                ShowMessage(_L("CAudioStreamEngine::SetEncodingL Encoding set to AMR-NB."), ETrue);  
                }
            }
        else
            {
            ...
            ShowMessage(_L("CAudioStreamEngine::SetEncodingL Encoding set to PCM."), ETrue); 
            }
...
void CAudioStreamEngine::MaoscPlayComplete(TInt aError)
    ...
    if (aError==KErrNone) 
        {
        // normal stream closure
        iAppUi->LogInfo(_L("CAudioStreamEngine::MaoscPlayComplete"));
        }   
    else if (aError==KErrUnderflow) 
        {
        // end of audio data stream was reached because of stream underflow,
        iAppUi->LogInfo(_L("CAudioStreamEngine::MaoscPlayComplete KErrUnderflow"));
        }
    else 
        {
        // completed with error(s)
        iAppUi->LogInfo(_L("CAudioStreamEngine::MaoscPlayComplete error:%d"), aError);
        }   
    }
