...
int DecompressFrame(unsigned char *cdata,int size,unsigned int *outdata,int outsize)
{
static int first=1,framenum=0;
		...
		/*if(yp!=NULL && up!=NULL && vp!=NULL){
			FILE *fpyuv;
			if (framenum==1)
				fpyuv=fopen("/sdcard/video.yuv","wb");
			else
				fpyuv=fopen("/sdcard/video.yuv","ab");
			if (fpyuv!=NULL){
				char sMsgLog[128];
				int iUVSize = (horizontal_size/2) * (vertical_size/2);
				fwrite(yp,1,horizontal_size * vertical_size, fpyuv);
				fwrite(up,1, iUVSize, fpyuv);
				fwrite(vp,1,iUVSize, fpyuv);
				fclose(fpyuv);
			}
		}*/
