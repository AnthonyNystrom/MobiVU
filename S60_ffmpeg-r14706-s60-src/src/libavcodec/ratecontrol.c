...
/*        rcc->last_qscale_for[FF_I_TYPE],
    ...
    const_values[ 6] = rce->pict_type == FF_B_TYPE ? (rce->f_code + rce->b_code)*0.5 : rce->f_code;
    const_values[ 7] = rce->i_count/mb_num;
    const_values[ 8] = rce->mc_mb_var_sum/mb_num;
    const_values[ 9] = rce->mb_var_sum/mb_num;
    const_values[10] = rce->pict_type == FF_I_TYPE;
    const_values[11] = rce->pict_type == FF_P_TYPE;
    const_values[12] = rce->pict_type == FF_B_TYPE;
    const_values[13] = rcc->qscale_sum[pict_type] / (double)rcc->frame_count[pict_type];
    const_values[14] = a->qcompress;
    const_values[15] = rcc->i_cplx_sum[FF_I_TYPE] / (double)rcc->frame_count[FF_I_TYPE];
    const_values[16] = rcc->i_cplx_sum[FF_P_TYPE] / (double)rcc->frame_count[FF_P_TYPE];
    const_values[17] = rcc->p_cplx_sum[FF_P_TYPE] / (double)rcc->frame_count[FF_P_TYPE];
    const_values[18] = rcc->p_cplx_sum[FF_B_TYPE] / (double)rcc->frame_count[FF_B_TYPE];
